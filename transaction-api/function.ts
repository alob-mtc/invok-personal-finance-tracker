...
async function getTransactions(
  ...
  try {
    ...
    const [transactionList, total] = await Promise.all([
      transactions.find(filter).sort(sort).skip(skip).limit(limit).toArray(),
...
async function updateTransaction(
  userId: string,
  transactionId: string,
  updates: Partial<TransactionRequest>
): Promise<TransactionResponse> {
  try {
    const db = await connectToDatabase();
    const transactions: Collection<Transaction> = db.collection("transactions");
      ...
      switch (method) {
        case "POST":
          ...
          const result = await createTransaction(
            userId,
            request.body as TransactionRequest
          );
        ...
        case "PUT":
          ...
          const updateResult = await updateTransaction(
            userId,
            query.id,
            request.body as Partial<TransactionRequest>
          );
